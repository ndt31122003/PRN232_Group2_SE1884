...
public sealed record ReserveStockCommand(
    ...
    InventoryReservationType ReservationType = InventoryReservationType.BuyItNow) : ICommand<ReserveStockResult>;
...
public sealed class ReserveStockCommandHandler : ICommandHandler<ReserveStockCommand, ReserveStockResult>
{
    ...
    public async Task<Result<ReserveStockResult>> Handle(ReserveStockCommand request, CancellationToken cancellationToken)
    {
        ...
        var reserveResult = inventory.ReserveStock(request.Quantity, buyerId, expiresAt, request.ReservationType);
        ...
        var reservation = inventory.Reservations
            .Where(x => x.BuyerId == buyerId && x.IsActive && x.Quantity == request.Quantity)
