...
public sealed record BulkDeleteDraftListingsCommand(
    List<Guid> ListingIds
) : ICommand;

public sealed class BulkDeleteDraftListingsCommandHandler : ICommandHandler<BulkDeleteDraftListingsCommand>
{
    private readonly IListingRepository _listingRepository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly IUserContext _userContext;

    public BulkDeleteDraftListingsCommandHandler(
    ...
    public async Task<Result> Handle(BulkDeleteDraftListingsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.ListingIds is null || request.ListingIds.Count == 0)
            return Error.Failure("Listing.EmptySelection", "Danh sách listing cần xóa không được để trống.");
        ...
        var listingsToRemove = new List<Listing>(distinctIds.Length);
        ...
        foreach (var listingId in distinctIds)
        {
            ...
            {
                return Error.Failure("Listing.InvalidStatus", "Chỉ có thể xóa hàng loạt các listing ở trạng thái draft.");
            }

            listingsToRemove.Add(listing);
        }
