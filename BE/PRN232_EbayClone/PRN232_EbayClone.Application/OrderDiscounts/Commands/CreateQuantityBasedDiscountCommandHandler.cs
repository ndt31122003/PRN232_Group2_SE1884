...
internal sealed class CreateQuantityBasedDiscountCommandHandler : ICommandHandler<CreateQuantityBasedDiscountCommand, Guid>
{
    private readonly IOrderDiscountRepository _repository;
    private readonly IUnitOfWork _unitOfWork;

    public CreateQuantityBasedDiscountCommandHandler(
    ...
    public async Task<Result<Guid>> Handle(CreateQuantityBasedDiscountCommand request, CancellationToken cancellationToken)
    {
        var sellerId = new UserId(request.SellerId);

        var discountResult = OrderDiscount.CreateQuantityBased(
            sellerId,
            request.Name,
            request.Description,
